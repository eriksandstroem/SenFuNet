    ...
    def forward(self, neighborhood):
        ...
        if not self.attend_to_uninit:
            if len(self.config.DATA.input) > 1:
                scores[:, :27] = scores[:, :27] - torch.where(weight['tof'] > 0, torch.zeros_like(scores[:, :27]), torch.ones_like(scores[:, :27]) * float('inf'))
                scores[:, 27:] = scores[:, 27:] - torch.where(weight['stereo'] > 0, torch.zeros_like(scores[:, 27:]), torch.ones_like(scores[:, 27:]) * float('inf'))
            else:
                scores[:, :] = scores[:, :] - torch.where(weight[sensor_] > 0, torch.zeros_like(scores[:, :]), torch.ones_like(scores[:, :]) * float('inf'))
