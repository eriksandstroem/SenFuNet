...
from modules.filtering_net import FilteringNet
...
class Pipeline(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        self._translation_network = TranslationNet(config)
    ...
    def _fusion(self, input_, input_features, values, fusionNet=None, featureNet=None):
        ...
        feature_est = feat_pred.view(b, h * w, n_points, self.config.FEATURE_MODEL.n_features)
    ...
    def _filtering(self, neighborhood): 
        if self.config.FILTERING_MODEL.fixed:
            with torch.no_grad():
                tsdf = self._filtering_network.forward(neighborhood)
        else:
            tsdf = self._filtering_network.forward(neighborhood)
    ...
    def _prepare_fusion_input(self, frame, values_sensor, values_sensor_op, confidence=None, n_points=None):
        ...
        features = values_sensor['feature_values']
        features = features.view(b, h, w, n_points * features.shape[-1])
        feature_weights_sensor = values_sensor['feature_weights'].view(b, h, w, n_points)
        # stacking input data
        feature_input = torch.cat([tsdf_frame, feature_weights_sensor, features], dim=3)
    ...
    def _prepare_volume_update(self, values, est, features, inputs, sensor):
    ...
    def _prepare_input_training(self, volume, weights_volume, bbox):
        ...
        if volume.dim() == 3: # tsdf
            neighborhood = volume[bbox[0, 0]:bbox[0, 1],
                bbox[1, 0]:bbox[1, 1],
                bbox[2, 0]:bbox[2, 1]]
        else: # features
            neighborhood = volume[bbox[0, 0]:bbox[0, 1],
        ...
        neighborhood = torch.cat([neighborhood.unsqueeze_(0), neighborhood_weights.unsqueeze_(0)], dim=0) # shape (C, D, H ,W)
    ...
    def _prepare_sensor_fusion_input(self, database, bbox):
        ...
        extract_tsdf = volume['current_tof'][bbox[0, 0]:bbox[0, 1],
                bbox[1, 0]:bbox[1, 1],
                bbox[2, 0]:bbox[2, 1]]
        ...
        neighborhood_filter['tof'] = torch.cat((neighborhood_tsdf.unsqueeze(0), neighborhood_weights.unsqueeze(0)), dim=0).unsqueeze(0).to(self.device) # add batch dimension

        neighborhood_tsdf = self.config.FUSION_MODEL.init_value*torch.ones(tuple(bbox[:, 1] - bbox[:, 0]))

        # even if we exceed the volume dimensions, we will only retrieve the largest possible box
        extract_tsdf = volume['current_stereo'][bbox[0, 0]:bbox[0, 1],
                bbox[1, 0]:bbox[1, 1],
                bbox[2, 0]:bbox[2, 1]]
        ...
        neighborhood_filter['stereo'] = torch.cat((neighborhood_tsdf.unsqueeze(0), neighborhood_weights.unsqueeze(0)), dim=0).unsqueeze(0).to(self.device) # add batch dimension
    ...
    def _prepare_sensor_fusion_input_old(self, scene, database, bbox):
        # TODO: do reflection padding if the central voxel is at the grid boundary. For now I do constant padding
        output = dict()
        neighborhood_filter = dict()
    ...
    def _prepare_local_grids(self, bbox, database, scene):
        ...
        tsdf_tof = database[scene]['current_tof'][bbox[0, 0]:bbox[0, 1],
                bbox[1, 0]:bbox[1, 1],
                bbox[2, 0]:bbox[2, 1]]
        ...
        tsdf_stereo = database[scene]['current_stereo'][bbox[0, 0]:bbox[0, 1],
                bbox[1, 0]:bbox[1, 1],
                bbox[2, 0]:bbox[2, 1]]
        ...
        local_grid_tof = torch.cat((tsdf_tof.unsqueeze(0), weights_tof.unsqueeze(0)), dim=0).unsqueeze(0) # add batch dimension
        local_grid_stereo = torch.cat((tsdf_stereo.unsqueeze(0), weights_stereo.unsqueeze(0)), dim=0).unsqueeze(0) # add batch dimension
    ...
    def fuse(self,
        ...
        values_sensor = eval('self._extractor_' + batch['sensor']).forward(frame,
                                         ...
                                         database[scene_id]['current' + '_' + batch['sensor']],
        ...
        values_sensor_op = eval('self._extractor_' + batch['sensor']).forward(frame,
                                         batch['extrinsics'],
                                         batch['intrinsics' + '_' + batch['sensor']],
                                         database[scene_id]['current' + '_' + batch['sensor_opposite']],
        ...
        values, features, weights, feature_weights, indices = self._integrator.forward(integrator_input,
                                                   database[scene_id][
                                                       'current_' + batch['sensor']].to(device),
                                                   database[scene_id][
    ...
    def fuse_training(self, batch, database, device):
        ...
        n_points = eval('self.config.FUSION_MODEL.n_points_' + batch['sensor'])
        tsdf_input, feature_input = self._prepare_fusion_input(frame, values_sensor, values_sensor_op, confidence, n_points)
        ...
        filtered_output = self._filter_training(indices, 
            ...
            database[scene_id]['current_' + batch['sensor_opposite']].to(device), 
            weights, 
        ...
        if filtered_output is not None:
            tsdf_filtered_grid = filtered_output[0]
            tsdf_gt_filtered_grid = filtered_output[1]
            tsdf_target_grid = filtered_output[2]
        else:
            return None
        ...
        tsdf_predicted_grid = tsdf_filtered_grid

        tsdf_predicted_grid = torch.clamp(tsdf_predicted_grid,
                            -self.config.DATA.trunc_value,
                            self.config.DATA.trunc_value)

        if tsdf_gt_filtered_grid is not None:
            tsdf_gt_filtered_grid = torch.clamp(tsdf_gt_filtered_grid,
                                -self.config.DATA.trunc_value,
                                self.config.DATA.trunc_value)

        output['tsdf_predicted_grid'] = tsdf_predicted_grid
        output['tsdf_gt_predicted_grid'] = tsdf_gt_filtered_grid
        output['tsdf_target_grid'] = tsdf_target_grid


        del tsdf_predicted_grid, tsdf_gt_filtered_grid, tsdf_target_grid
        return output
            ...
            for j in range(math.ceil(2*y_size/chunk_size) - 1):
                ...
                for k in range(math.ceil(2*z_size/chunk_size) - 1):
                    ...
                    with torch.no_grad():
                        # input_['neighborhood_filter']['tof'][0, 0, :, :, :] = -0.0*torch.ones_like(input_['neighborhood_filter']['tof'][0, 0, :, :, :])
                        # input_['neighborhood_filter']['tof'][0, 1, :, :, :] = 0.0*torch.ones_like(input_['neighborhood_filter']['tof'][0, 1, :, :, :])
                        sub_tsdf_filter = self._filtering(input_)
                    ...
                    filtered_local_grid[moving_bbox[0, 0] + int(chunk_size/4):moving_bbox[0, 1] - int(chunk_size/4),
                                        ...
                                        moving_bbox[2, 0] + int(chunk_size/4):moving_bbox[2, 1] - int(chunk_size/4)] = sub_tsdf[0, 0,
    ...
    def _filter_training(self, indices, gt_vol, tsdf_vol, tsdf_vol_op, weights_vol, weights_vol_op, sensor, sensor_op, device):
        ...
        input_1 = self._prepare_input_training(tsdf_vol, weights_vol, bbox) 

        input_2 = self._prepare_input_training(tsdf_vol_op, weights_vol_op, bbox) 
        ...
        if self.config.LOSS.gt_loss:
            input_1 = self._prepare_input_training(gt_vol.to(device), self.config.FUSION_MODEL.max_weight*torch.ones_like(weights_vol), bbox) 

            input_2 = self._prepare_input_training(gt_vol.to(device), self.config.FUSION_MODEL.max_weight*torch.ones_like(weights_vol), bbox) 
        ...
        # mask filtered loss
        tsdf_filtered = tsdf_filtered[:, :, valid_indices[:, 0], valid_indices[:, 1], valid_indices[:, 2]]
        if self.config.LOSS.gt_loss:
            tsdf_gt_filtered = tsdf_gt_filtered[:, :, valid_indices[:, 0], valid_indices[:, 1], valid_indices[:, 2]].squeeze()
        ...
        return tsdf_filtered.squeeze(), tsdf_gt_filtered, tsdf_target.squeeze()
