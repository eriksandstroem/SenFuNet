...
class Database(Dataset):
    ...
    def __init__(self, dataset, config):
        ...
        for s in dataset.scenes:
            ...
            self.tsdf_tof[s] = Voxelgrid(self.scenes_gt[s].resolution)
            self.tsdf_tof[s].from_array(init_volume1, self.scenes_gt[s].bbox)
            self.fusion_weights_tof[s] = np.zeros(self.scenes_gt[s].volume.shape, dtype=np.float16)
            self.tsdf_stereo[s] = Voxelgrid(self.scenes_gt[s].resolution)
            self.tsdf_stereo[s].from_array(init_volume2, self.scenes_gt[s].bbox)
    ...
    def __getitem__(self, item):
        ...
        sample['tsdf_tof'] = self.tsdf_tof[item].volume
        sample['tsdf_stereo'] = self.tsdf_stereo[item].volume
    ...
    def filter(self, value=2.):
        ...
        for key in self.tsdf.keys():

            weights = self.fusion_weights[key]
            self.tsdf_tof[key].volume[weights < value] = self.initial_value
            self.fusion_weights_tof[key][weights < value] = 0
            self.tsdf_stereo[key].volume[weights < value] = self.initial_value
    ...
    def save_to_workspace(self, workspace, is_best, is_best_tof, is_best_stereo, save_mode='ply'):
        ...
        for key in self.scenes_gt.keys():
            ...
            tsdf_volume_tof = self.tsdf_tof[key].volume
            weight_volume_tof = self.feature_weights_tof[key]

            tsdf_volume_stereo = self.tsdf_stereo[key].volume
    ...
    def save(self, path, save_mode='ply', scene_id=None):
        ...
        if save_mode =='ply':
            ...
            voxel_size = 0.01
            vertices, faces, normals, _ = skimage.measure.marching_cubes_lewiner(self.tsdf_tof[scene_id].volume, 
            ...
            filename = os.path.join(path, ply_file)
            vertices, faces, normals, _ = skimage.measure.marching_cubes_lewiner(self.tsdf_stereo[scene_id].volume, 
                    level=0, spacing=(voxel_size, voxel_size, voxel_size))
            mesh = trimesh.Trimesh(vertices=vertices, faces=faces, vertex_normals=normals)
            mesh.export(filename)
            ...
            with h5py.File(os.path.join(path, filename), 'w') as hf:
                hf.create_dataset("TSDF",
                                      shape=self.tsdf_tof[scene_id].volume.shape,
                                      data=self.tsdf_tof[scene_id].volume,
                                      compression='gzip',
                                      compression_opts=9)
            ...
            with h5py.File(os.path.join(path, filename), 'w') as hf:
                hf.create_dataset("TSDF",
                                      shape=self.tsdf_stereo[scene_id].volume.shape,
                                      data=self.tsdf_stereo[scene_id].volume,
                                      compression='gzip',
                                      compression_opts=9)
            with h5py.File(os.path.join(path, weightname), 'w') as hf:
                hf.create_dataset("weights",
    ...
    def evaluate(self, mode='train', workspace=None):
        ...
        for scene_id in self.scenes_gt.keys():
            ...
            est_tof = self.tsdf_tof[scene_id].volume
            est_stereo = self.tsdf_stereo[scene_id].volume
    ...
    def reset(self, scene_id=None):
        if scene_id:
            ...
            self.tsdf_tof[scene_id].volume = self.initial_value * np.ones(self.tsdf_tof[scene_id].volume.shape, dtype=np.float16)
            self.fusion_weights_tof[scene_id] = np.zeros(self.tsdf_tof[scene_id].volume.shape, dtype=np.float16)
            self.tsdf_stereo[scene_id].volume = self.initial_value * np.ones(self.tsdf_stereo[scene_id].volume.shape, dtype=np.float16)
            self.fusion_weights_stereo[scene_id] = np.zeros(self.tsdf_stereo[scene_id].volume.shape, dtype=np.float16)
            ...
            self.feature_weights_tof[scene_id] = np.zeros(self.tsdf_stereo[scene_id].volume.shape, dtype=np.float16)
            self.feature_weights_stereo[scene_id] = np.zeros(self.tsdf_stereo[scene_id].volume.shape, dtype=np.float16)
            self.filtered[scene_id].volume = self.initial_value * np.ones(self.tsdf_tof[scene_id].volume.shape, dtype=np.float16)
        else:
            for scene_id in self.scenes_gt.keys():
                feature_shape = (self.scenes_gt[scene_id].volume.shape[0], self.scenes_gt[scene_id].volume.shape[1], self.scenes_gt[scene_id].volume.shape[2], self.n_features)
                self.tsdf_tof[scene_id].volume = self.initial_value * np.ones(self.tsdf_tof[scene_id].volume.shape, dtype=np.float16)
