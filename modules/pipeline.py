...
class Pipeline(torch.nn.Module):
    ...
    def test_tsdf(self, val_loader, val_dataset, val_database, sensors, device):
        ...
        if self.config.FILTERING_MODEL.model == 'tsdf_early_fusion':
            ...
            for scene in val_database.filtered.keys():
                ...
                val_database.filtered[scene].volume = np.divide(val_database.filtered[scene].volume, weight_mask, out=np.zeros_like(weight_mask), where=weight_mask!=0.0)

                # here I don't divide by 2 everywhere, but 1 where we only have 1 sensor integration
                val_database.sensor_weighting[scene][:, :, :] = 0.5 # this is not correct. It should be 0.5 and 0 and 1, but I don't care about this now

        elif self.config.FILTERING_MODEL.model == 'tsdf_plain_average_andmask_fusion': # simple average fusion
            for scene in val_database.filtered.keys():
                and_mask = np.ones_like(val_database.filtered[scene].volume)
                for sensor_ in sensors: 
                    and_mask = np.logical_and(and_mask, val_database.fusion_weights[sensor_][scene] > 0)
                    val_database.filtered[scene].volume += val_database.tsdf[sensor_][scene].volume
                val_database.filtered[scene].volume /= len(sensors) #this can shift the outliers more i.e. we take the average
                # even if only one sensor integrates compared to the middle fusion case where we don't consider 
                # uninitialized voxels in the average. But we cannot get rid of the outliers though....
                for sensor_ in sensors:
                    val_database.feature_weights[sensor_][scene] = and_mask.astype(np.float32)
                val_database.sensor_weighting[scene][:, :, :] = 0.5

    def test_step(self, batch, database, sensors, device): # used for trajectory performance plot
    ...
    def test_speed(self, val_loader, val_dataset, val_database, sensors, device): # used to evaluate speed of algorithm
        ...
        for k, batch in tqdm(enumerate(val_loader), total=len(val_dataset)):
            ...
            if self.filter_pipeline is not None:
                # run filtering network on all voxels which have a non-zero weight
                for scene in val_database.filtered.keys():   
                    self.filter_pipeline.filter(scene, val_database, device) # it would be good 
    ...
    def test_step_video(self, batch, database, sensor, sensors, device): # used for video creation
