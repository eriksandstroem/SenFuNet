...
def train_fusion(args):
    ...
    if config.FILTERING_MODEL.features_to_sdf_enc or config.FILTERING_MODEL.features_to_weight_head:
        ...
        optimizer_feature = torch.optim.Adam(feature_params,
                                            config.OPTIMIZATION.lr_fusion,
                                            betas=(0.9, 0.999),
    ...
    if not config.FILTERING_MODEL.fixed and pipeline.filter_pipeline is not None:
        ...
        optimizer_filt = torch.optim.Adam(pipeline.filter_pipeline._filtering_network.parameters(),
                                            config.OPTIMIZATION.lr_filtering,
                                            betas=(0.9, 0.999),
                                            eps=1e-08, weight_decay=0)
    ...
    is_best = dict()
    for sensor in config.DATA.input:
        best_iou[sensor] = 0. # USE THIS FOR REAL TRAINING
                ...
                if param.grad is not None:
                    ...
                    if (i + 1) % config.OPTIMIZATION.accumulation_steps == 0 or i == n_batches - 1:
                        if name.startswith('fuse_pipeline._feature'):
                            ...
                            param.grad)
                        elif name.startswith('filter_pipeline._filtering_network.encoder') or name.startswith('filter_pipeline._filtering_network.sdf'):
                            grad_norm_outlier_net[name.split('.')[3]] += torch.norm(
                            param.grad)
                        else:
                            grad_norm_alpha_net += torch.norm(
                            param.grad)
...
# 
                ...
                workspace.writer.add_scalar('Train/grad_norm_alpha_net', grad_norm_alpha_net, global_step=i + 1 + epoch*n_batches)
                ...
                for sensor_ in config.DATA.input:
                    ...
                    workspace.writer.add_scalar('Train/grad_norm_outlier_net' + sensor_, grad_norm_outlier_net[sensor_], global_step=i + 1 + epoch*n_batches)
                    ...
                    workspace.writer.add_scalar('Train/l_alpha_2d_' + sensor_, l_alpha_2d[sensor_], global_step=i + 1 + epoch*n_batches)
            ...
            if (i + 1) % config.OPTIMIZATION.accumulation_steps == 0 or i == n_batches - 1:
                ...
                if not config.FILTERING_MODEL.fixed:
                    ...
                    for name, param in pipeline.named_parameters():
                        if param.grad is not None:
                            if name.startswith('filter_pipeline._filtering_network.weight_decoder'):
            ...
            # if False:
            if (i + 1) % config.SETTINGS.eval_freq == 0 or i == n_batches - 1 or (i == 2 and epoch == 0):  # evaluate after 20 steps wince then we have integrated at least one frame for each scene
