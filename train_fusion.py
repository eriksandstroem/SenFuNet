    ...
    for epoch in range(0, config.TRAINING.n_epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), total=len(train_dataset)):
            ...
            if config.TRAINING.clipping:
                # if config.DATA.fusion_strategy == 'three_fusionNet':
                #     torch.nn.utils.clip_grad_norm_(pipeline._fusion_network_mono.parameters(),
                #                                    max_norm=1.,
                #                                    norm_type=2)   
                #     torch.nn.utils.clip_grad_norm_(pipeline._fusion_network_stereo.parameters(),
                #                                    max_norm=1.,
                #                                    norm_type=2) 
                #     torch.nn.utils.clip_grad_norm_(pipeline._fusion_network_tof.parameters(),
                #                                    max_norm=1.,
                #                                    norm_type=2)                       
                # else:
