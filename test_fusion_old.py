...
def test_fusion(config):
   ...
   # load pretrained routing model into parameters
    ...
    loading.load_pipeline(config.TESTING.fusion_model_path, pipeline) # this loads all parameters available in the checkpoint that can be loaded
    ...
    loading.load_fusion_net_old(config.TRAINING.pretraining_fusion_tof_model_path, pipeline, 'tof')
    loading.load_fusion_net_old(config.TRAINING.pretraining_fusion_stereo_model_path, pipeline, 'stereo')
