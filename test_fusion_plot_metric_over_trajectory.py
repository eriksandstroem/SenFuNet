...
def test_fusion(config):
    ...
    print(
        "Filtering Net: ", count_parameters(pipeline.filter_pipeline._filtering_network)
    )
    ...
    sensors = config.DATA.input  # ['tof', 'stereo']
    ...
    for i, batch in tqdm(enumerate(loader), total=len(dataset)):
        ...
        (
            ...
            eval_results_fused,
            eval_results_scene,
            eval_results_scene_fused,
        ) = database.evaluate(mode="test")
        ...
        for scene in database.scenes_gt.keys():
            for sensor_ in l1[scene].keys():
                ...
                # write to txt file
                np.savetxt(
                    test_dir + "/" + "mad_" + sensor_ + ".txt", l1[scene][sensor_]
                )
                np.savetxt(
                    test_dir + "/" + "mse_" + sensor_ + ".txt", l2[scene][sensor_]
                )
                np.savetxt(
                    test_dir + "/" + "iou_" + sensor_ + ".txt", iou[scene][sensor_]
                )
                np.savetxt(
                    test_dir + "/" + "acc_" + sensor_ + ".txt", acc[scene][sensor_]
                )
