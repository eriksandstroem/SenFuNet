...
def arg_parse():
    parser = argparse.ArgumentParser(description='Script for creating a video of the depth.')
...
def get_depth(sensor, scene, trajectory, dataset):
    if dataset == 'replica':
        input_dir = '/cluster/work/cvl/esandstroem/data/replica/manual/' + scene + '/' + trajectory + '/' + sensor
    else:
        if sensor == 'tof':
            # corbs
            # input_dir = '/cluster/work/cvl/esandstroem/data/corbs/human/data/H1_pre_registereddata/depth'
            # scene3d
            input_dir = '/cluster/work/cvl/esandstroem/data/scene3d/copyroom/copyroom_png/depth'
        else:
            # corbs
    ...
    for k, im in enumerate(images):
        ...
        plt.imsave(output_folder + '/' + '%04d'%k + '.png',\
            np.asarray(im), vmin=0, vmax=5, dpi = 1)
...
# vmin=0, vmax=25000
    ...
    os.system('ffmpeg -framerate 15 -i %04d.png -vcodec libx264 -preset veryslow -c:a libmp3lame -r 15 -crf 25 -pix_fmt yuv420p ' + '/'.join(output_folder.split('/')[:-1]) + '.mp4')
