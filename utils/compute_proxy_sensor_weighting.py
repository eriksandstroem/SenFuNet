...
def compute_proxy_sensor_weighting_and_mesh(tsdfs, gt_tsdf, test_dir, weights, voxel_size, truncation, scene):
	...
	mask_between = np.logical_or(np.logical_and((tsdfs[sensors[0]] < gt_tsdf), (gt_tsdf < tsdfs[sensors[1]])), \
	...
	filtered_tsdf = sensor_weighting*tsdfs[sensors[0]] + (1 - sensor_weighting)*tsdfs[sensors[1]]
	...
	plt.savefig(test_dir + '/proxy_sensor_weighting_surface_histogram.png') # here I see that the averaged alpha values in the neighborhood skews the
