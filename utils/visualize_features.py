...
def visualize_features(
    ...
    # To speed up the cpu accelerated t-SNE version, we remove
    # points that we feed to the t-SNE algorithm. This is done by downsampling the grids to get a
    # larger voxel size. With voxel size 1 cm, there are around 12M samples to be embedded. With downsampling
    # factor 2 this drops significantly.
    ...
    #     voxel_points = np.round(np.asarray(vertices - v_size / 2) * 1 / v_size).astype(
    #         int
    #     )
    ...
    if mc == "Open3D":
        ...
        (verts, faces, normals, values) = skimage.measure.marching_cubes_lewiner(
            fused_tsdf,
            level=0,
            spacing=(v_size, v_size, v_size),
            mask=preprocess_weight_grid(mask),
        )
        # add 0.5 * voxel_size to vertices to match Open3D marching cubes output
        vertices = verts + v_size / 2
        mesh = o3d.geometry.TriangleMesh(
    ...
    voxel_points = np.round(np.asarray(vertices - v_size / 2) * 1 / v_size).astype(int)
    ...
    # this plots the error of the fused result
