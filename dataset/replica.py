...
class Replica(Dataset):
    ...
    def _get_scene_load_order(self, nbr_load_scenes):
        ...
        if nbr_load_scenes > len(scenes_list):
            raise ValueError(
                "nbr_load_scenes variable is lower than the number of scenes"
            )
        ...
        while scenes_list:
            if nbr_load_scenes > len(scenes_list):
                scene_indices = random.sample(
                    range(0, len(scenes_list)), len(scenes_list)
                )
            else:
                scene_indices = random.sample(
                    range(0, len(scenes_list)), nbr_load_scenes
                )
            ...
            scenes_list = [
                val for idx, val in enumerate(scenes_list) if idx not in scene_indices
            ]
    ...
    def _load_depth_gt(
        ...
        if self.scenedir is not None:
            ...
            for key in self.scenedir:
                ...
                for trajectory in self.scenedir[key]:
                    files = glob.glob(
                        os.path.join(self.root_dir, trajectory, modality, "*.png")
                    )
                    files = sorted(
                        files, key=lambda x: int(os.path.splitext(x.split("/")[-1])[0])
                    )
                    for file in files:
                        tmp_list.append(file)
                # replace the short list with the long list including all file paths
            ...
            with open(os.path.join(self.root_dir, self.scene_list), "r") as file:
                for line in file:
                    ...
                    if (
                        ...
                        self._scenes.append(
                            line[self.sensor_line_mapping["left_depth_gt"]].split("/")[
                                0
                            ]
                        )
                    files = glob.glob(
    ...
    def _load_cameras(self):
        ...
        if self.scenedir is not None:
            ...
            for key in self.scenedir:
                ...
                for trajectory in self.scenedir[key]:
                    files = glob.glob(
                        os.path.join(self.root_dir, trajectory, modality, "*.txt")
                    )
                    files = sorted(
                        files, key=lambda x: int(os.path.splitext(x.split("/")[-1])[0])
                    )
            ...
            with open(os.path.join(self.root_dir, self.scene_list), "r") as file:
                ...
                for line in file:
                    line = line.split(" ")
                    files = glob.glob(
                        os.path.join(self.root_dir, line[-1][:-1], "*.txt")
                    )
                    for file in files:
