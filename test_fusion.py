...
def test_fusion(config):
    ...
    if config.FILTERING_MODEL.model != 'tsdf_middle_fusion':
        loading.load_pipeline(config.TESTING.fusion_model_path, pipeline)
        ...
        for weight_threshold in weight_thresholds:
            ...
            if config.TESTING.routedfusion_nn:
                ...
                for sensor_ in config.DATA.input:
                    ...
                    weights_path = config.SETTINGS.experiment_path + '/' + \
                    config.TESTING.routedfusion_nn_model + test_path + '/' + scene + '_' + sensor_ + '.weights.hf5'
